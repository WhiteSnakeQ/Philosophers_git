...
// static int	calt_time(int mod, t_philo *philo)
// {
// 	int	time;
// 	int	ret;

// 	if (mod == 1)
// 		time = philo->mother->t_sleep;
// 	else
// 		time = philo->mother->t_eat;
// 	ret = time + philo->last_act - get_time(philo->mother, 2);
// 	if (mod == 0 && ret < 0)
// 		return (time * 2 + philo->last_act - get_time(philo->mother, 2));
// 	if (ret < 0)
// 		return (0);
// 	return (ret);
// }
